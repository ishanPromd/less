...
import { EnhancedFeedPage } from './EnhancedFeedPage';
...
export const UserDashboard: React.FC = () => {
  ...
  const handleTabChange = useCallback((tabId: string) => {
    setActiveTab(tabId);
    // Reset navigation state when changing tabs, but preserve feed state
    if (tabId !== 'feed') {
      setSelectedSubject(null);
      setSelectedPaper(null);
    }
    
    // Force refresh of lesson data when switching to lesson-related tabs
    if (tabId === 'lessons' || tabId === 'my-lessons') {
      // Trigger a small delay to ensure components are mounted before refreshing
      setTimeout(() => {
        window.dispatchEvent(new Event('refreshLessonData'));
      }, 100);
    }
  }, []);

  const handlePlayerModalToggle = useCallback((isOpen: boolean) => {
  ...
  // Removed profile page

  // Show lessons page when lessons tab is active
  if (activeTab === 'lessons') {
    return <LessonsPage onNavigate={handleTabChange} activeTab={activeTab} />;
  }

  // Show my lessons page when my-lessons tab is active
  if (activeTab === 'my-lessons') {
    return <EnhancedFeedPage key="my-lessons-reset" onNavigate={handleTabChange} activeTab={activeTab} onPlayerModalToggle={handlePlayerModalToggle} />;
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="w-full px-4 py-6 pb-24">
        {/* Home Tab - Main View */}
        {activeTab === 'home' && !selectedSubject && !selectedPaper && (
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h2 className="text-xl font-bold mb-3">
                      <TypingText text="Tecnology A/L Premium" className="bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent" />
                    </h2>
                    <p className="text-blue-100 mb-4 text-sm">
                      Advanced Level Excellence Platform
                    </p>
                  </div>
                  <Button 
                    variant="outline" 
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button 
                ...
                size="sm"
                onClick={signOut}
                className="text-red-600 border-red-300 hover:bg-red-50 text-xs px-3 py-1.5 lg:hidden"
              >
                <FontAwesomeIcon icon={faSignOutAlt} className="w-3 h-3 mr-1" />
                Sign Out
              </Button>
              ...
              {recentQuizzes.length > 0 ? (
                recentQuizzes.map((quiz, index) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        <div className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center border-2 border-gray-200 group-hover:border-blue-300 transition-colors">
                      ...
                      <motion.div
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.95 }}
                        className="w-10 h-10 bg-gray-800 rounded-full flex items-center justify-center group-hover:bg-blue-600 transition-colors shadow-lg"
                      >
                        <FontAwesomeIcon icon={faArrowRight} className="w-4 h-4 text-white" />
                      </motion.div>
                    </div>
                ...
                <div className="text-center py-8">
                  ...
                  <h3 className="text-base font-medium text-gray-900 mb-2">No Recent Papers</h3>
                  <p className="text-gray-600 text-sm">
        ...
        {activeTab === 'notifications' && (
          <div className="space-y-6">
            ...
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              className="relative overflow-hidden rounded-3xl bg-gradient-to-r from-purple-500 via-purple-600 to-indigo-700 p-6 text-white"
            >
              <div className="absolute top-4 right-4 w-12 h-12 bg-white/10 rounded-full"></div>
              <div className="absolute top-7 right-7 w-6 h-6 bg-white/20 rounded-full"></div>
              <div className="absolute bottom-4 right-10 w-3 h-3 bg-white/15 rounded-full"></div>
              ...
              <div className="relative z-10">
                <div className="flex items-start justify-between mb-3">
                  ...
                  <Button 
                    ...
                    size="sm"
                    onClick={signOut}
                    className="bg-white/20 border-white/30 text-white hover:bg-white/30 backdrop-blur-sm text-xs px-3 py-1.5 ml-2 lg:hidden"
                  >
                    <FontAwesomeIcon icon={faSignOutAlt} className="w-3 h-3 mr-1" />
                    Sign Out
                  </Button>
